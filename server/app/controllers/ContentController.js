...
async function createContent(contentType, category, name, description, director, actors, owner, videoUrl, videoPreview,  country, token) {
    try {
        ...
        return await db.query(`
                ...
                (name, description, owner, type, director, country, actors, video_url, category, date, user_id, video_preview)
                VALUES ('${name}','${description}', '${owner}', '${contentType}', '${director}', 
                '${country}', '${actors}', '${videoUrl}', '${category}', '${date}', ${user.id}, '${videoPreview}')
                RETURNING id
           `)
    } catch (e) {
