...
export function StepCard({number, title}: IStepCard) {
    ...
    return (<div className={stepStyle}>
            <div className="w-[120px] h-[200px] bg-[#1d2b37] rotate-45 absolute -m-10 rounded-lg overflow-hidden" />
            <div className="text-7xl text-start relative mt-4 ml-8 font-bold text-[#ffb81c] z-10">{number}</div>
            <p className="text-start absolute bottom-12 left-10 text-white text-2xl font-bold">{title}</p>
        </div>)
}